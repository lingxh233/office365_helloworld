...
function advanced_google_recaptcha_get_captcha_api_url( $type ) {
	...
	if ( 'v2' === $type ) {
		$output = 'https://www.recaptcha.net/recaptcha/api.js?hl=' . esc_attr( get_locale() ) . '&onload=agr_load&render=explicit';
	} elseif ( 'v3' === $type ) {
		$output = 'https://www.recaptcha.net/recaptcha/api.js?onload=agr_v3&render=' . esc_attr( $site_key );
	}
